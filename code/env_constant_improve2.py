...
class Env_Constant_improve_Catan2(gym.Env):
    ...
    def step(self, action_index: int):
        ...
        self.choice = 0
        # self.choice→0：city,1：settlement,2：road,3：other
        ...
        for currPlayer in self.playerQueue.queue:
            ...
            if(currPlayer.name == "player1"):
                ...
                # print(f"self.city_check{self.city_check}")
                # print(f"self.settle_check{self.settle_check}")
                # print(f"self.road_check{self.road_check}")
                # print(self.board.get_potential_cities(
                #     self.playerQueue.queue[0]))
            ...
            if currPlayer.victoryPoints >= self.maxPoints:
                ...
                # print(f"make_road：{self.make_road}")
                # print(f"make_settle：{self.make_settle}")
                # print(f"make_city：{self.make_city}")
                if(currPlayer.name == "player1"):
                    reward += 500000
                Over_Flag = True
                p_v = currPlayer.name
                ...
                if(self.game_count == 1):
                    f = open('result_constant_improve2.txt', 'w')
                    f.write(f"{currPlayer.name}\n")
                else:
                    f = open('result_constant_improve2.txt', 'a')
                    f.write(f"{currPlayer.name}\n")
                self.game_count += 1
        ...
        onehot = self.can_roads_set_city(self.playerQueue.queue[0])
        ...
        observation = np.append(observation, np.array([onehot]))
        ...
        # print(f"potential_set_city: {potential_set_city}")
        # print(f"potential_road: {potential_road}")
    ...
    def reset(self):
       ...
       # 発展カード
        ...
        observation = np.append(observation, np.array([np.zeros(3)]))
        observation = np.append(observation, np.array([my_resource]))
    ...
    def OBSERVATION_SPACE(self):
        ...
        OBS_SPACE = spaces.Dict({
            ...
            "can_onehot": spaces.Box(
                low=np.array(np.zeros(3)),
                high=np.array(np.full(3, 1)),  # 最大枚数は後で調整
                dtype=np.uint8
            ),
