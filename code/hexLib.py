...
# Define the x y point for pixels(画素のx y点を定義する)
...
# Add function to create Hex from Axial Coordinates and an axial point as input(軸座標と軸点を入力としてHexを作成する機能の追加)


def Axial_Hex(axialPoint):
    s = -axialPoint.q-axialPoint.r
    assert not (round(axialPoint.q + axialPoint.r + s)
                != 0), "q + r + s must be 0"
...
# Linear interpolation between 2 hexes(2つのヘックス間の線形補間)
...
# Specify Orientation and layout for Hex <-> Pixel conversion(Hex <-> Pixel 変換のための方向とレイアウトを指定する)
...
# Layout has the orientation, size and origin(レイアウトは、方向、サイズ、原点を持つ)
Layout = collections.namedtuple("Layout", ["orientation", "size", "origin"])

# Function to covert axial hex coordinates to pixel(軸方向の16進座標を画素に変換する機能)
...
# Function to convert pixel coordinates to Hex(画素の座標を16進数に変換する関数)
...
# Get the corner offset depending on the layout (using start angle)(レイアウトに応じたコーナーオフセットを取得（スタートアングルを使用）)
...
# Get the corners of the Polygon in pixel coordinates(ポリゴンのコーナーをピクセル座標で取得する)
...
def polygon_corners(layout, h):
    ...
    for i in range(0, 6):
        ...
        corners.append(Point(round(center.x + offset.x, 2),
                       round(center.y + offset.y, 2)))
    return corners
