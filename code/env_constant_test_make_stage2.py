...
class Env_Constant_Test_Make_Stage2(gym.Env):
    ...
    def step(self, action_index: int):
        ...
        for currPlayer in self.playerQueue.queue:
            ...
            elif(currPlayer != 'player1' and self.change == True):
            ...
            if currPlayer.victoryPoints >= self.maxPoints:
                ...
                #     f = open('result_constant.txt', 'w')
                #     f.write(f"{currPlayer.name}\n")
                # else:
                #     f = open('result_constant.txt', 'a')
                #     f.write(f"{currPlayer.name}\n")
                self.game_count += 1
        ...
        pick_up = [[robber_position],  road.tolist(),
                   settle_city.tolist(), my_resource.tolist()]
        print(pick_up)
    ...
    def reset(self):
        ...
        # self.build_newgame(
        #     self.STAGE_LIST[random.randint(0, len(self.STAGE_NUM))])
        # new_resource = resource_dict[random.randint(0, 5)]
        # pick_resource = random.choice(new_resource)
        # print(pick_resource)

        # self.build_newgame(self, num[self.game_count])
    ...
    def build_newgame(self, list):
        ...
        self.board.updateBoardGraph_robber(list[self.game_count][0])
        road_list = list[self.game_count][1]
        s_c_list = list[self.game_count][2]
        for i in range(len(road_list)):
            road_tuple = self.save_road_dic[i]
            edge1 = self.vertex_index_to_pixel_dict[road_tuple[0]]
            edge2 = self.vertex_index_to_pixel_dict[road_tuple[1]]
    ...
    def get_stage(self):
        stage = []
        a = ""
        f = open('board_info3.txt', 'r', encoding='UTF-16')
        data = f.read().split("\n")
        skip = False
        for i in data:
            list_all = []
