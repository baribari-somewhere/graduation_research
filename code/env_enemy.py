...
from myAI import *
from env_vs_single import Env_Vs_Single
import queue
import numpy as np
import sys
import pygame
...
class Env_Enemy(gym.Env):
    ACTION_MAP = np.array(ACTION)
    # PLAYER_MODEL1: DQN = DQN.load(
    #     "./code/save_weights_constant/rl_model_70000000_steps")
    # PLAYER_MODEL2: DQN = DQN.load(
    #     "./code/save_weights_constant/rl_model_70000000_steps")
    # PLAYER_MODEL3: DQN = DQN.load(
    #     "./code/save_weights_constant/rl_model_70000000_steps")
    ...
    def __init__(self):
        ...
        self.player_env: Env_Vs_Single = Env_Vs_Single()
        self.obs = None
    ...
    def step(self, action_index: int):
        ...
        for currPlayer in self.playerQueue.queue:
            ...
            if(currPlayer.name == 'player1'):
                ...
                # print(f"/n/n/n/n/n/n/n/n{action}/n/n/n/n/n/n/n")
                self.obs, rewards, dones, info = self.player_env.step(
                    action_index, currPlayer)

                reward = self.point
                # print("a")
            if(currPlayer.name == "player2"):
                # print("b")
                # action, _state = Env_Enemy.PLAYER_MODEL1.predict(self.obs)
                
                # self.dicerolled = False
                currPlayer.move(self.board)
            if(currPlayer.name == "player3"):
                # print("c")
                # action, _state = Env_Enemy.PLAYER_MODEL2.predict(self.obs)
                # self.dicerolled = False
                currPlayer.move(self.board)
            if(currPlayer.name == "player4"):
                # print("d")
                # action, _state = Env_Enemy.PLAYER_MODEL3.predict(self.obs)
                # self.dicerolled = False
                currPlayer.move(self.board)
            ...
            if currPlayer.victoryPoints >= self.maxPoints:
                ...
                if(self.game_count == 1):
                    f = open('result_vs.txt', 'w')
                    f.write(f"{currPlayer.name}\n")
                else:
                    f = open('result_vs.txt', 'a')
                    f.write(f"{currPlayer.name}\n")
                self.game_count += 1
