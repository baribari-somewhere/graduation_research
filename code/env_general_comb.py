...
class Env_General_comb(gym.Env):
    ...
    def step(self, action_index: int):
        ...
        if self.playerQueue.queue[self.figure].victoryPoints >= self.maxPoints:
            if(self.playerQueue.queue[self.figure].name == "player1" or self.playerQueue.queue[self.figure].name == "player3"):
                reward += 500000
            Over_Flag = True
            # p_v = self.playerQueue.queue[self.figure].name
            ...
            if(self.game_count == 1):
                f = open('result_general_comb.txt', 'w')
                f.write(
                    f"{self.playerQueue.queue[self.figure].name}\n")
            else:
                f = open('result_general_comb.txt', 'a')
                f.write(
                    f"{self.playerQueue.queue[self.figure].name}\n")
            self.game_count += 1
        ...
        dev = self.DV_sort(self.figure, list1)
        ...
        for i in my_resource:
            list2.append(i)
        resource = self.RESOURCE_sort(self.figure, list2)
        ...
        observation = np.append(observation, np.array([resource]))
        observation = np.append(observation, np.array([dev]))
    ...
    def RESOURCE_sort(self, num, list1):
        if(num == 0):
            for i in range(5):
                top = list1.pop(0)
                list1.append(top)
