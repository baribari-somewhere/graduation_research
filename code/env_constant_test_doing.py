...
class Env_Constant_Test_Doing(gym.Env):
    ...
    def build_newgame(self, list):
        ...
        for j in range(len(s_c_list)):
            ...
            if (s_c_list[j] == 1):
                ...
                self.playerQueue.queue[0].build_city(
                    build_position, self.board)
            elif (s_c_list[j] == 3):
                self.playerQueue.queue[1].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 4):
                self.playerQueue.queue[1].build_city(
                    build_position, self.board)
            elif (s_c_list[j] == 5):
                self.playerQueue.queue[2].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 6):
                self.playerQueue.queue[2].build_city(
                    build_position, self.board)
            elif (s_c_list[j] == 7):
                self.playerQueue.queue[3].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 8):
                self.playerQueue.queue[3].build_city(
                    build_position, self.board)
    ...
    def action_input(self, action, Player):
        ...
        if(self.game_count == 999):
            print(f"average：{self.total_point/self.game_count}")
    ...
    def get_stage(self):
        stage = []
        a = ""
        f = open('stage_info2.txt', 'r', encoding='UTF-8-sig')
        data = f.read().split("\n")
        skip = False
        for i in data:
            a = ""
