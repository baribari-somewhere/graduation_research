...
from resource_list import resource_dict
import collections


class Env_Constant_Test4_Get_Resource_Board(gym.Env):
    ACTION_MAP = np.array(ACTION)
    # STAGE_LIST = self.get_stage()
    ...
    def __init__(self):
        ...
        # index 0：パスを選ぶ　1：道を作ろうとしたが無理　2：道を作った　3：開拓地が無理だった
        # 4：開拓地を作った　5：都市が無理だった　6：都市を作った
        ...
        self.STAGE_LIST = self.get_stage()
        self.STAGE_DIC = self.separate(self.STAGE_LIST)
        print(f"STAGE_DIC[0]:{len(self.STAGE_DIC[0])}")
        print(f"STAGE_DIC[1]:{len(self.STAGE_DIC[1])}")
        print(f"STAGE_DIC[2]:{len(self.STAGE_DIC[2])}")
        print(f"STAGE_DIC[3]:{len(self.STAGE_DIC[3])}")
        print(f"STAGE_DIC[4]:{len(self.STAGE_DIC[4])}")
        print(f"STAGE_DIC[5]:{len(self.STAGE_DIC[5])}")
        print(f"STAGE_DIC[6]:{len(self.STAGE_DIC[6])}")
        print(
            f"total：{len(self.STAGE_DIC[0])+len(self.STAGE_DIC[1])+len(self.STAGE_DIC[2])+len(self.STAGE_DIC[3])+len(self.STAGE_DIC[4])+len(self.STAGE_DIC[5])+len(self.STAGE_DIC[6])}")
        # # print(self.STAGE_DIC[0][0])
        # self.STAGE_NUM = len(self.STAGE_LIST)
    ...
    def step(self, action_index: int):
        ...
        # self.build_initial_settlements2()
        # self.build_newgame(self.STAGE_LIST[self.game_count])
        # print(self.STAGE_LIST[random.randint(0, self.STAGE_NUM)])

        # self.build_newgame(
        #     self.STAGE_LIST[random.randint(0, self.STAGE_NUM)-1])

        # for i in self.playerQueue.queue:
        #     # new_resource = resource_dict[random.randint(0, 5)]
        #     new_resource = resource_dict[1]
        #     pick_resource = random.choice(new_resource)
        #     i.resources["ORE"] = pick_resource[0]
        #     i.resources["BRICK"] = pick_resource[1]
        #     i.resources["WHEAT"] = pick_resource[2]
        #     i.resources["WOOD"] = pick_resource[3]
        #     i.resources["SHEEP"] = pick_resource[4]

        # print(f"action_index:{action_index}")

        # self.board.updateBoardGraph_robber(num[0][0])

        for currPlayer in self.playerQueue.queue:
            if(currPlayer.name == "player1"):
                # print(currPlayer.resources['WHEAT'])
                # print(currPlayer.resources['ORE'])
                if((True in self.board.get_potential_cities(
                        self.playerQueue.queue[0]).values()) == True and currPlayer.resources['WHEAT'] >= 2 and currPlayer.resources['ORE'] >= 3):

                    self.city_check = True
                else:
                    self.city_check = False
                if((True in self.board.get_potential_settlements(
                        self.playerQueue.queue[0]).values()) == True and currPlayer.resources['BRICK'] > 0 and currPlayer.resources['WOOD'] > 0 and currPlayer.resources['SHEEP'] > 0 and currPlayer.resources['WHEAT'] > 0):
                    self.settlement_check = True
                else:
                    self.settlement_check = False
                if((True in self.board.get_potential_roads(
                        self.playerQueue.queue[0]).values()) == True and currPlayer.resources['BRICK'] > 0 and currPlayer.resources['WOOD'] > 0):
            ...
            if(currPlayer.name == 'player1'):
                ...
                # print(currPlayer.victoryPoints)
                # currPlayer.move(self.board)
                self.change = True

                self.action_input(action, currPlayer)
                # print(self.correct_c)
                # print(action)

                reward = self.point
                # print(f"reward_before: {reward}")

                if(self.city_check == False and self.settlement_check == False and self.road_check == False):
                    self.list1[self.action_check(action)] += 1
                    self.list_all[0][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list1[5][0] += 1
                        self.list_all[0][5][0] += 1
                    elif(self.correct_s):
                        self.list1[5][1] += 1
                        self.list_all[0][5][1] += 1
                    elif(self.correct_c):
                        self.list1[5][2] += 1
                        self.list_all[0][5][2] += 1
                elif(self.city_check == True and self.settlement_check == False and self.road_check == False):
                    self.list2[self.action_check(action)] += 1
                    self.list_all[1][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list2[5][0] += 1
                        self.list_all[1][5][0] += 1
                    elif(self.correct_s):
                        self.list2[5][1] += 1
                        self.list_all[1][5][1] += 1
                    elif(self.correct_c):
                        self.list2[5][2] += 1
                        self.list_all[1][5][2] += 1
                elif(self.city_check == False and self.settlement_check == True and self.road_check == False):
                    self.list3[self.action_check(action)] += 1
                    self.list_all[2][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list3[5][0] += 1
                        self.list_all[2][5][0] += 1
                    elif(self.correct_s):
                        self.list3[5][1] += 1
                        self.list_all[2][5][1] += 1
                    elif(self.correct_c):
                        self.list3[5][2] += 1
                        self.list_all[2][5][2] += 1
                elif(self.city_check == False and self.settlement_check == False and self.road_check == True):
                    self.list4[self.action_check(action)] += 1
                    self.list_all[3][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list4[5][0] += 1
                        self.list_all[3][5][0] += 1
                    elif(self.correct_s):
                        self.list4[5][1] += 1
                        self.list_all[3][5][1] += 1
                    elif(self.correct_c):
                        self.list4[5][2] += 1
                        self.list_all[3][5][2] += 1
                elif(self.city_check == True and self.settlement_check == True and self.road_check == False):
                    self.list5[self.action_check(action)] += 1
                    self.list_all[4][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list5[5][0] += 1
                        self.list_all[4][5][0] += 1
                    elif(self.correct_s):
                        self.list5[5][1] += 1
                        self.list_all[4][5][1] += 1
                    elif(self.correct_c):
                        self.list5[5][2] += 1
                        self.list_all[4][5][2] += 1
                elif(self.city_check == True and self.settlement_check == False and self.road_check == True):
                    self.list6[self.action_check(action)] += 1
                    self.list_all[5][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list6[5][0] += 1
                        self.list_all[5][5][0] += 1
                    elif(self.correct_s):
                        self.list6[5][1] += 1
                        self.list_all[5][5][1] += 1
                    elif(self.correct_c):
                        self.list6[5][2] += 1
                        self.list_all[5][5][2] += 1
                elif(self.city_check == False and self.settlement_check == True and self.road_check == True):
                    self.list7[self.action_check(action)] += 1
                    self.list_all[6][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list7[5][0] += 1
                        self.list_all[6][5][0] += 1
                    elif(self.correct_s):
                        self.list7[5][1] += 1
                        self.list_all[6][5][1] += 1
                    elif(self.correct_c):
                        self.list7[5][2] += 1
                        self.list_all[6][5][2] += 1
                elif(self.city_check == True and self.settlement_check == True and self.road_check == True):
                    self.list8[self.action_check(action)] += 1
                    self.list_all[7][self.action_check(action)] += 1
                    if(self.correct_r):
                        self.list8[5][0] += 1
                        self.list_all[7][5][0] += 1
                    elif(self.correct_s):
                        self.list8[5][1] += 1
                        self.list_all[7][5][1] += 1
                    elif(self.correct_c):
                        self.list8[5][2] += 1
                        self.list_all[7][5][2] += 1

            # elif(currPlayer.name != 'player1'):
            #     # print(f"move: {currPlayer.name}")
            #     currPlayer.move(self.board)
            #     self.dicerolled = False
        ...
        potential_set_city = self.get_potential_thing(
            self.playerQueue.queue[0])
        potential_road = self.get_potential_road(self.playerQueue.queue[0])

        can_road = self.can_road(self.playerQueue.queue[0])
        can_settlement = self.can_settlement(self.playerQueue.queue[0])
        can_city = self.can_city(self.playerQueue.queue[0])
    ...
    def reset(self):
        ...
        select_stage_dic = self.STAGE_DIC[random.randint(0, 6)]
        select_stage = select_stage_dic[random.randint(
            0, len(select_stage_dic)-1)]
        self.build_newgame(select_stage)
    ...
    def separate(self, stage_list):
        ...
        for i in stage_list:
            board = constant_catanBoard()
            # print(self.test_counter)
            if(self.test_counter % 1000 == 0):
                print(self.test_counter)
                # print(i[0])
            self.test_counter += 1
            playerQueue = queue.Queue(self.numPlayers)
            ...
            for figure in range(4):
                ...
                newPlayer = heuristicAIPlayer(
                    f"player{figure+1}", playerColors[figure])
                newPlayer.updateAI()
                playerQueue.put(newPlayer)
                # self.player_set.append(newPlayer)
            # player_set.append(newPlayer)
            for j in playerQueue.queue:
                j.resources["ORE"] = 100
                j.resources["BRICK"] = 100
                j.resources["WHEAT"] = 100
                j.resources["WOOD"] = 100
                j.resources["SHEEP"] = 100
            # ={'ORE': 5, 'BRICK': 6,
            #    'WHEAT': 3, 'WOOD': 6, 'SHEEP': 3}
            # print(list[self.game_count%self.STAGE_NUM][1])
            # print(self.game_count%self.STAGE_NUM)
            # print(list[0][0])
            board.updateBoardGraph_robber(i[0][0])
            road_list = i[1]
            # print(road_list)
            s_c_list = i[2]
            # road_list = list[self.game_count%self.STAGE_NUM][1]
            # s_c_list = list[self.game_count%self.STAGE_NUM][2]
            for k in range(len(road_list)):
                road_tuple = self.save_road_dic[k]
                edge1 = board.vertex_index_to_pixel_dict[road_tuple[0]]
                edge2 = board.vertex_index_to_pixel_dict[road_tuple[1]]

                if (road_list[k] == 1):
                    playerQueue.queue[0].build_road(
                        edge1, edge2, board)
                elif (road_list[k] == 2):
                    playerQueue.queue[1].build_road(
                        edge1, edge2, board)
                elif (road_list[k] == 3):
                    playerQueue.queue[2].build_road(
                        edge1, edge2, board)
                elif (road_list[k] == 4):
                    playerQueue.queue[3].build_road(
                        edge1, edge2, board)
            for l in range(len(s_c_list)):
                build_position = board.vertex_index_to_pixel_dict[l]

                if (s_c_list[l] == 1):
                    playerQueue.queue[0].build_settlement(
                        build_position, board)
                elif (s_c_list[l] == 2):
                    playerQueue.queue[0].build_city_init(
                        build_position, board)
                elif (s_c_list[l] == 3):
                    playerQueue.queue[1].build_settlement(
                        build_position, board)
                elif (s_c_list[l] == 4):
                    playerQueue.queue[1].build_city_init(
                        build_position, board)
                elif (s_c_list[l] == 5):
                    playerQueue.queue[2].build_settlement(
                        build_position, board)
                elif (s_c_list[l] == 6):
                    playerQueue.queue[2].build_city_init(
                        build_position, board)
                elif (s_c_list[l] == 7):
                    playerQueue.queue[3].build_settlement(
                        build_position, board)
                elif (s_c_list[l] == 8):
                    playerQueue.queue[3].build_city_init(
                        build_position, board)

            # print(playerQueue.queue[0].resources)
            # if(True in board.get_potential_cities(
            #         playerQueue.queue[0]).values()):
            #     # print(board.get_potential_roads(
            #     #     playerQueue.queue[0]))

                # for i in stage_list:
                #     if((i[3][3] == 0 or i[3][1] == 0) and (i[3][0] <= 2 or i[3][2] <= 1)):
                #         list1.append(i)
                #     elif((i[3][0] >= 3 and i[3][2] >= 2) and (i[3][1] == 0 or i[3][3] == 0)):
                #         list2.append(i)
                #     elif((i[3][1] >= 1 and i[3][3] >= 1) and (i[3][2] == 0 or i[3][4] == 0) and (i[3][0] <= 2 or i[3][2] <= 1)):
                #         list3.append(i)
                #     elif((i[3][0] >= 3 and i[3][2] >= 2) and (i[3][1] >= 1 and i[3][2] >= 1 and i[3][3] >= 1 and i[3][4] >= 1)):
                #         list4.append(i)
                #     elif((i[3][0] >= 3 and i[3][2] >= 2) and (i[3][1] >= 1 and i[3][3] >= 1)):
                #         list5.append(i)
                #     elif(i[3][1] >= 1 and i[3][2] >= 1 and i[3][3] >= 1 and i[3][4] >= 1):
                #         list6.append(i)
                #     else:
                #         list7.append(i)
            ...
            if((i[3][3] == 0 or i[3][1] == 0) and (i[3][0] <= 2 or i[3][2] <= 1)):
                # print("a")
                list1.append(i)
            elif((i[3][0] >= 3 and i[3][2] >= 2) and (i[3][1] == 0 or i[3][3] == 0) and True in board.get_potential_cities(
                    playerQueue.queue[0]).values()):
                list2.append(i)
                # print("b")
            elif((i[3][1] >= 1 and i[3][3] >= 1) and (i[3][2] == 0 or i[3][4] == 0) and (i[3][0] <= 2 or i[3][2] <= 1) and True in board.get_potential_roads(
                    playerQueue.queue[0]).values()):
                list3.append(i)
                # print("c")
            elif((i[3][0] >= 3 and i[3][2] >= 2) and (i[3][1] >= 1 and i[3][2] >= 1 and i[3][3] >= 1 and i[3][4] >= 1) and True in board.get_potential_cities(
                    playerQueue.queue[0]).values() and True in board.get_potential_settlements(
                        playerQueue.queue[0]).values()):
                list4.append(i)
                # print("d")
            elif((i[3][0] >= 3 and i[3][2] >= 2) and (i[3][1] >= 1 and i[3][3] >= 1) and True in board.get_potential_cities(
                    playerQueue.queue[0]).values() and True in board.get_potential_roads(
                        playerQueue.queue[0]).values()):
                list5.append(i)
                # print("e")
            elif(i[3][1] >= 1 and i[3][2] >= 1 and i[3][3] >= 1 and i[3][4] >= 1 and True in board.get_potential_settlements(
                    playerQueue.queue[0]).values() and True in board.get_potential_roads(
                        playerQueue.queue[0]).values()):
                list6.append(i)
                # print("f")
        ...
        resource_dict[6] = list7

        return resource_dict

    def build_newgame(self, list):

        resource = list[3]

        for i in self.playerQueue.queue:
            i.resources["ORE"] = 100
            i.resources["BRICK"] = 100
            i.resources["WHEAT"] = 100
            i.resources["WOOD"] = 100
            i.resources["SHEEP"] = 100
            # ={'ORE': 5, 'BRICK': 6,
            #    'WHEAT': 3, 'WOOD': 6, 'SHEEP': 3}
        # print(list[self.game_count%self.STAGE_NUM][1])
        # print(self.game_count%self.STAGE_NUM)
        # print(list[0][0])
        self.board.updateBoardGraph_robber(list[0][0])
        road_list = list[1]
        # print(road_list)
        s_c_list = list[2]
        # road_list = list[self.game_count%self.STAGE_NUM][1]
        ...
        for i in range(len(road_list)):
            road_tuple = self.save_road_dic[i]
            edge1 = self.board.vertex_index_to_pixel_dict[road_tuple[0]]
            edge2 = self.board.vertex_index_to_pixel_dict[road_tuple[1]]

            if (road_list[i] == 1):
                self.playerQueue.queue[0].build_road(edge1, edge2, self.board)
            elif (road_list[i] == 2):
                self.playerQueue.queue[1].build_road(edge1, edge2, self.board)
            elif (road_list[i] == 3):
                self.playerQueue.queue[2].build_road(edge1, edge2, self.board)
            elif (road_list[i] == 4):
                self.playerQueue.queue[3].build_road(edge1, edge2, self.board)
        for j in range(len(s_c_list)):
            build_position = self.board.vertex_index_to_pixel_dict[j]

            if (s_c_list[j] == 1):
                self.playerQueue.queue[0].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 2):
                self.playerQueue.queue[0].build_city_init(
                    build_position, self.board)
            elif (s_c_list[j] == 3):
                self.playerQueue.queue[1].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 4):
                self.playerQueue.queue[1].build_city_init(
                    build_position, self.board)
            elif (s_c_list[j] == 5):
                self.playerQueue.queue[2].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 6):
                self.playerQueue.queue[2].build_city_init(
                    build_position, self.board)
            elif (s_c_list[j] == 7):
                self.playerQueue.queue[3].build_settlement(
                    build_position, self.board)
            elif (s_c_list[j] == 8):
                self.playerQueue.queue[3].build_city_init(
                    build_position, self.board)

        for i in self.playerQueue.queue:
            for j in range(0, 3):
                i.resources["ORE"] = resource[0+j*5]
                i.resources["BRICK"] = resource[1+j*5]
                i.resources["WHEAT"] = resource[2+j*5]
                i.resources["WOOD"] = resource[3+j*5]
                i.resources["SHEEP"] = resource[4+j*5]
    ...
    def can_settlement2(self, player):
        if((True in self.board.get_potential_settlements(
                player).values() == True) and player.resources['BRICK'] > 0 and player.resources['WOOD'] > 0 and player.resources['SHEEP'] > 0 and player.resources['WHEAT'] > 0):
            return True
        else:
    ...
    def get_stage(self):
        stage = []
        a = ""
        f = open('stage_resource.txt', 'r', encoding='UTF-8-sig')
        data = f.read().split("\n")
        skip = False
        counter = 0
        ...
        for i in data:
            ...
            for j in i:
                ...
                # elif(j == ','):
                    ...
                    a = ""
                    count += 1
                elif(j == "," and count == 127):
                    list_all.append(list2)
                    a = ""
                    count += 1
                elif(j == "," and 127 < count and count <= 146):
                    list3.append(int(a))
                    a = ""
                    count += 1
                elif(count == 147 and j == "]"):
                    list3.append(int(a))
