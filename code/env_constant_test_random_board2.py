...
class Env_Constant_Test_Random_Board2(gym.Env):
    ...
    def step(self, action_index: int):
        ...
        # print(currPlayer.resources)
        if((True in self.board.get_potential_cities(
                self.playerQueue.queue[0]).values()) == True and self.playerQueue.queue[0].resources['WHEAT'] >= 2 and self.playerQueue.queue[0].resources['ORE'] >= 3):

            self.city_check = True
        else:
            self.city_check = False
        if((True in self.board.get_potential_settlements(
                self.playerQueue.queue[0]).values()) == True and self.playerQueue.queue[0].resources['BRICK'] > 0 and self.playerQueue.queue[0].resources['WOOD'] > 0 and self.playerQueue.queue[0].resources['SHEEP'] > 0 and self.playerQueue.queue[0].resources['WHEAT'] > 0):
            self.settlement_check = True
        else:
            self.settlement_check = False
        if((True in self.board.get_potential_roads(
                self.playerQueue.queue[0]).values()) == True and self.playerQueue.queue[0].resources['BRICK'] > 0 and self.playerQueue.queue[0].resources['WOOD'] > 0):
        ...
        self.action_input(action, self.playerQueue.queue[0])
    ...
    def reset(self):
        ...
        # Functiont to go through initial set up（初期セットアップを行うための関数）
        self.build_initial_settlements2()
        # print(random.randint(0, self.STAGE_NUM)-1)
        self.build_newgame(
            self.STAGE_LIST[random.randint(0, self.STAGE_NUM)-1])
        ...
        # self.playerQueue.queue[0].resources["ORE"] = 100
        # self.playerQueue.queue[0].resources["BRICK"] = 100
        # self.playerQueue.queue[0].resources["WHEAT"] = 100
        # self.playerQueue.queue[0].resources["WOOD"] = 100
        # self.playerQueue.queue[0].resources["SHEEP"] = 100
    ...
    def get_stage(self):
        stage = []
        a = ""
        f = open('stage_info5.txt', 'r', encoding='UTF-8-sig')
        data = f.read().split("\n")
        skip = False
        for i in data:
            a = ""
