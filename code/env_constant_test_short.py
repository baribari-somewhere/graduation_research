...
class Env_Constant_Test_Short(gym.Env):
    ...
    def step(self, action_index: int):
        ...
        for currPlayer in self.playerQueue.queue:
            if(currPlayer.name == "player1"):
                ...
                if(len(self.board.get_potential_cities(
                        self.playerQueue.queue[0])) >= 1 and currPlayer.resources['WHEAT'] >= 2 and currPlayer.resources['ORE'] >= 3):

                    self.city_check = True
                else:
                    self.city_check = False
                if(len(self.board.get_potential_settlements(
                        self.playerQueue.queue[0])) >= 1 and currPlayer.resources['BRICK'] > 0 and currPlayer.resources['WOOD'] > 0 and currPlayer.resources['SHEEP'] > 0 and currPlayer.resources['WHEAT'] > 0):
                    self.settlement_check = True
                else:
                    self.settlement_check = False
                if(len(self.board.get_potential_roads(
                        self.playerQueue.queue[0])) >= 1 and currPlayer.resources['BRICK'] > 0 and currPlayer.resources['WOOD'] > 0):
    ...
    def OBSERVATION_SPACE(self):
        ...
        OBS_SPACE = spaces.Dict({
            ...
            # "can_set&city": spaces.Box(
            #     low=np.array(np.zeros(1)),
            #     high=np.array(np.full(1, 2)),  # 最大枚数は後で調整
            #     dtype=np.uint8
            # ),
