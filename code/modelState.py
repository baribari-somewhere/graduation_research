...
class modelState():
    ...
    RLモデルで必要な変数をstateとactionオブジェクトで定義する
    状態 
    頂点。長さ54の配列。0は空、-1/-2は敵、+1/+2は自己（集落/都市）を表す。
    エッジ/ロード。長さ ___ の配列、空白は 0、敵は -1、自分は +1 を使用。
    勝利ポイント。num_playersの長さの配列、0から10の間の値を使用。
    HexTiles。リソースと個数を持つタプル配列
    NumPlayerCards: プレイヤー数num_playerのカード。num_playersの長さの配列で、各プレイヤーが持つカードの合計枚数（デベロップカードとリソースカード）。
    強盗の位置。強盗の現在地 - hexTile

    アクション: (リソースの制約の下で)
    建設 都市、集落、道路を建設します。
    開発カードを引く 開発カードを1枚引く
    開発カードをプレイします。開発カードを1枚出す
    交易：銀行（港のオプションあり）や他のプレイヤーと交易します。
