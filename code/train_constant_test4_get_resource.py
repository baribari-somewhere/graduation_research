...
from env_constant_test3 import *
from env_constant_test4_get_resource import *
...
# model = DQN.load("./save_weights_test_random_20230112/rl_model_8000000_steps.zip",
#                  env, verbose=1, tensorboard_log="log", device="auto")
model = DQN.load("./save_weights_test4_get_resource_20230113_conti10000000-12000000/rl_model_59000000_steps.zip",
                 env, verbose=1, tensorboard_log="log", device="auto")
...
#     save_freq=setting.default_save_freq, save_path="save_weights_test4_get_resource_20230123_conti10000000-12000000-59000000")
...
for i in range(1000):
    if(i != 0):
        model = DQN.load(
            f"./tmp/get_resource_dqn_{num*(i)}", env, verbose=1, tensorboard_log="log", device="auto")
    else:
        model = DQN.load("./save_weights_test4_get_resource_20230113_conti10000000-12000000/rl_model_59000000_steps.zip",
                         env, verbose=1, tensorboard_log="log", device="auto")
    model.learn(num)
    model.save(f"./tmp/get_resource_dqn_{num*(i+1)}")
    del model
